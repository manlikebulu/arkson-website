...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    fetch(form.action, {
      ...
      .then(response => {
        if (response.ok) {
          ...
          return response.json().then(data => {
            const errMsg = (data && data.errors && data.errors.map(er => er.message).join(', '))
      ...
      .catch(err => {
        status.textContent = err.message || 'Something went wrong sending that. Try again, or email us directly.';
        status.className = 'form-status error';
      })
      .finally(() => {
